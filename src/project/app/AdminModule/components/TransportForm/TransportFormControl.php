<?php
declare(strict_types=1);

namespace App\AdminModule\Components\TransportForm;

use Exception;
use Nette\Application\AbortException;
use Nette\Application\UI\Control;
use Nette\Application\UI\Form;
use Tomaj\Form\Renderer\BootstrapRenderer;

class TransportFormControl extends Control
{

	/**
	 * @var Handler $handler
	 */
	private $handler;

	/**
	 * @var callable
	 */
	private $onSuccess;

	/**
	 * @var callable
	 */
	private $onError;

	public function __construct(Handler $handler, callable $onSuccess, callable $onError)
	{
		parent::__construct();
		$this->handler = $handler;
		$this->onSuccess = $onSuccess;
		$this->onError = $onError;
	}

	public function render()
	{
		$template = $this->getTemplate();
		$template->setFile(__DIR__ . '/form.latte');
		$template->render();
	}

	public function createComponentForm(): Form
	{
		$form = new Form();
		$form->setRenderer(new BootstrapRenderer());
		$form->addHidden('id');

		$form->addText('name', 'Name')
			 ->setRequired('Transport name is required parameter');

		$form->addTextArea('description', 'Description')
			 ->setRequired('Transport description is required parameter');

		$form->addUpload('photo', 'Cover image');

		$form->addCheckbox('active', 'Active');

		$form->addText('capacity', 'Capacity')
			 ->addRule(Form::INTEGER)
			 ->setRequired('Capacity is required');

		$form->addText('luggage', 'Luggage')
			 ->addRule(Form::INTEGER)
			 ->setRequired('Luggage is required');

		$form->addText('fixedPrice', 'Fixed price (€)')
			 ->addRule(Form::FLOAT)
			 ->setRequired('Fixed price is required');

		$form->addText('multiplierPrice', 'Multiplier Price')
			 ->addRule(Form::FLOAT)
			 ->setRequired('MultiplierPrice is required');

		$form->addSubmit('send', 'Save');

		$form->onSuccess[] = function (Form $form) {
			$this->onSuccess($form);
		};
		return $form;
	}

	private function onSuccess(Form $form): void
	{
		$values = $form->getValues(true);

		try {
			$this->handler->handle($values);

			($this->onSuccess)();
		} catch (Exception $exception) {
			if ($exception instanceof AbortException) {
				throw $exception;
			}
			($this->onError)();
		}
	}
}
